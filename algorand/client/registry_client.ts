...
export function toAlgorandSolventRegistryPayload(
  epoch: CanonicalEpochObject
): AlgorandRegistryPayload {
  const healthStatus =
    HEALTH_STATUS_STRING_MAP[epoch.health_status.toUpperCase()] ??
    HealthStatus.UNKNOWN;

  const scaleAmount = (usd: number): bigint =>
    BigInt(Math.round(usd * Number(AMOUNT_SCALE)));
...
export function encodeState(
  payload: AlgorandRegistryPayload,
  insolvencyFlag: boolean,
  liquidityStressFlag: boolean
): Uint8Array {
 ...
 */
export function decodeState(raw: Uint8Array): EpochRecord {
  ...
  const health_status = view.getUint8(offset++) as HealthStatus;
...
export interface RegistryClientConfig {
  /** Algorand node URL, e.g. "https://testnet-api.algonode.cloud" */
  nodeUrl: string;
  /** Algorand node token (use "" for public nodes) */
  nodeToken?: string;
  /** Algorand node port (default 443) */
  nodePort?: number;
  /** On-chain application ID of the deployed SolventRegistry contract */
  appId: bigint;
  /** Signer used for state-changing transactions (submit_epoch) */
  signer?: algosdk.TransactionSigner;
  /** Sender address for state-changing transactions */
...
export class SolventRegistryClient {
  ...
  private readonly signer?: algosdk.TransactionSigner;
   ...
   */
  async submitEpoch(payload: AlgorandRegistryPayload): Promise<string> {
    if (!this.signer || !this.senderAddress) {
      throw new Error(
        "submitEpoch requires signer and senderAddress in client config"
      );
    }
    ...
    const abiMethod = algosdk.ABIMethod.fromSignature(
      "submit_epoch(string,string,string,string,string,string,uint64,uint64,uint64,bool,bool,uint8,uint64,uint64)void"
    );
   ...
   */
  async getLatestState(entityId: string): Promise<LatestState | null> {
   ...
   */
  async getHealthStatus(entityId: string): Promise<HealthStatus> {
    const state = await this.getLatestState(entityId);
  ...
  private async _readBox(boxKey: Uint8Array): Promise<EpochRecord | null> {
    try {
      ...
      const raw = response.value as Uint8Array;
...
// ============================================================
export type { AlgorandRegistryPayload, EpochRecord, LatestState };
