...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    const start = async () => {
      if (!enableWebcam || !modelsReady || !webcamRequested) return;
  ...
  }, [enableWebcam, modelsReady, depthResponse, webcamRequested]);

  return (
    <div ref={containerRef} className={`relative w-full h-full overflow-hidden ${className ?? ''}`} style={style}>
      {showPreview && enableWebcam && (
        <div className="absolute right-3 bottom-3 w-[220px] rounded-lg overflow-hidden border border-white/25 shadow-[0_4px_16px_rgba(0,0,0,0.4)] bg-black text-white text-[12px] leading-[1.2] font-sans pointer-events-auto z-50">
          {!webcamRequested ? (
            <button
              onClick={() => setWebcamRequested(true)}
              className="w-full h-[100px] flex flex-col items-center justify-center gap-2 hover:bg-white/10 transition-colors cursor-pointer"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3z"/>
                <circle cx="12" cy="13" r="3"/>
              </svg>
              <span className="text-[11px] opacity-80">Click to enable face tracking</span>
            </button>
          ) : webcamError ? (
            <div className="w-full h-[100px] flex flex-col items-center justify-center gap-2 text-red-400">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="12" cy="12" r="10"/>
                <line x1="15" y1="9" x2="9" y2="15"/>
                <line x1="9" y1="9" x2="15" y2="15"/>
              </svg>
              <span className="text-[11px]">{webcamError}</span>
              <button
                onClick={() => { setWebcamError(null); setWebcamRequested(true); }}
            ...
            <>
              <video ref={videoRef} muted playsInline autoPlay className="w-full h-[132px] object-cover -scale-x-100" />
